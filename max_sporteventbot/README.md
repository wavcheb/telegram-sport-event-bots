...
### Один человек в двух мессенджерах

Дежурства считаются по человеку, а не по аккаунту. Но бот не знает, что
аккаунт в MAX и аккаунт в Telegram принадлежат одному игроку, пока их не
связали: до этого он считается двумя разными участниками, и дежурство,
отработанное с одной стороны, не засчитывается с другой.

Связывается один раз самим игроком:

1. `/iam` в одном мессенджере — бот выдаёт код
2. `/iam КОД` в другом мессенджере **со своего аккаунта** — готово

После этого дежурства обоих аккаунтов суммируются, в списке участников
игрок учитывается один раз, а `/duty_stats` показывает одну строку.
Разорвать связь — `/iam_forget`.

Код одноразовый, но кто его введёт, тот и свяжется с вами, поэтому
пересылать его не стоит.
...
## 💰 Касса и страница оплат

`/event_bank` показывает, сколько сейчас в кассе, `/event_bank 5200` — записывает
новую сумму, `/event_bank 5200 после аренды` — с комментарием. Значение общее
для связанных чатов, история изменений сохраняется.

**Валюта своя у каждого чата** — одним ботом могут пользоваться команды из
разных стран. Укажите её один раз вместе с суммой, дальше бот её помнит:

```
/event_bank 120000 ₸          → 120 000 ₸
/event_bank 5200 KZT          → тем же способом, кодом ISO
/event_bank 5200 тенге        → распознаются и слова: рублей, тенге, гривен…
/event_bank 98000             → останется в ₸, повторять метку не нужно
```

Пересчёта между валютами нет и не будет: метка нужна только чтобы подписать
сумму. `BANK_CURRENCY` в `.env` — лишь запасной вариант для чатов, где валюту
ещё ни разу не указывали (по умолчанию `₽`).

Страница оплат (`PAYMENTS_PAGE_URL`) дополнительно показывает сумму в кассе и
таблицу дежурств за период — по умолчанию 4 недели, переключается ссылками
2/4/8/12 недель или параметром `?weeks=`.

Ссылки на страницу подписываются автоматически, ключ свой у каждого чата
(генерируется при первой ссылке и хранится в `Chats.page_secret`). Поэтому если
ботом пользуются несколько команд, ссылка одной группы не открывает событие
другой: подпись пришлось бы взять из чужого ключа. Настраивать ничего не нужно.

Это защита от подбора `?event=`, а не пароль: у кого есть сама ссылка, тот
видит это событие. Если ссылка ушла лишнему человеку, поменяйте ключ чата —
`UPDATE Chats SET page_secret = NULL WHERE chat_id = ...;` — и все прежние
ссылки этого чата перестанут работать, а следующая команда выдаст новую.
