...
Ссылки на страницу можно подписывать: задайте одинаковый `PAYMENTS_SECRET` в
`.env` обоих ботов и в `web/config.php`, и страница перестанет открываться по
подобранному `?event=`. Это защита от перебора, а не доступ по паролю: у кого
есть ссылка, тот видит событие.
